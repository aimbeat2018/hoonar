...
class LocalVideoSelectedScreen extends StatefulWidget {
  final String? duration;

  const LocalVideoSelectedScreen({super.key, this.duration});

  @override
  State<LocalVideoSelectedScreen> createState() =>
      _LocalVideoSelectedScreenState();
}

class _LocalVideoSelectedScreenState extends State<LocalVideoSelectedScreen> {
  ...
  Future<void> getSoundList(BuildContext context) async {
    ...
    sessionManager.initPref().then((onValue) async {
      ...
      await contestProvider.getSavedSoundList(requestModel,
          sessionManager.getString(SessionManager.accessToken) ?? '');

      if (contestProvider.errorMessage != null) {
        SnackbarUtil.showSnackBar(context, contestProvider.errorMessage ?? '');
      } else {
        if (contestProvider.savedSoundListModel?.status == '200') {
        } else if (contestProvider.savedSoundListModel?.message ==
            'Unauthorized Access!') {
          SnackbarUtil.showSnackBar(
              context, contestProvider.savedSoundListModel?.message! ?? '');
                              ...
                              : AnimatedList(
                                  ...
                                  itemBuilder: (context, index, animation) {
                                    return buildSoundItem(
                                      contestProvider
                                          .savedSoundListModel!.data![index],
                                      index,
                                      myLoading.isDark,
